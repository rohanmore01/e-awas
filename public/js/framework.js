...
jQuery(document).ready(function () {
    jQuery("#edit-search-block-form--2").attr(
        "placeholder",
        "Search - Keyword, Phrase"
    );
    jQuery(".gtranslate select").attr("id", "gtranslate");
    jQuery("#gtranslate").before(
        '<label class="notdisplay" for="gtranslate">Google Translate</label>'
    );
    //contrast
...
jQuery(document).ready(function () {
    ...
    jQuery("table.views-table tbody tr").each(function () {
        for (var j = 1; j <= count; j++) {
            jQuery("td:nth-child(" + j + ")", this).attr(
                "data-label",
                params[j - 1]
            );
        }
